...
const codeRoot=path.resolve(dir,'..');
const projectRoot=path.basename(codeRoot).toLowerCase()==='scripts'?path.dirname(codeRoot):codeRoot;
const evidencePath=process.argv[2] || path.join(projectRoot,'output_file','销量规律','分析证据.json');
const cleanupDefaultEvidence=!process.argv[2];
const outputDir=process.argv[3] || path.join(projectRoot,'output','_file');
